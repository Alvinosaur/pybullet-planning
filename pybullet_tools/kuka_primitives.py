...
GRASP_INFO = {
    'top': GraspInfo(lambda body: get_top_grasps(body, under=False, tool_pose=Pose(), max_width=INF, grasp_length=0),
...
def get_ik_fn(robot, fixed=[], teleport=False, num_attempts=10):
    # tolerance for iteratively updating IK solver
    movable_joints = get_movable_joints(robot)
    sample_fn = get_sample_fn(robot, movable_joints)

    def fn(body, pose, grasp, custom_limits={}, ik_kwargs={}):
        obstacles = [body] + fixed
        # world_grasp = world_block * inv(grasp_block)
        # grasp.grasp_pose defined wrt block pose (pose.pose), where
        # pose.pose defined wrt robot body
        # gripper_pose: helps define grasp pose wrt robot body frame
        gripper_pose = end_effector_from_body(pose.pose, grasp.grasp_pose)

        # world_approach = world_grasp * grasp_approach
        ...
        for _ in range(num_attempts):
            ...
            q_approach = inverse_kinematics(robot, grasp.link, approach_pose,
                                            max_iterations=200,
                                            **ik_kwargs, custom_limits=custom_limits)
