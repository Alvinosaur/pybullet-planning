...
def load_model(rel_path, is_abs_path=False, pose=None, **kwargs):
    # TODO: error with loadURDF when loading MESH visual and CYLINDER collision
    abs_path = rel_path if is_abs_path else get_model_path(rel_path)
    add_data_path()
    # with LockRenderer():
...
def inverse_kinematics(robot, link, target_pose, max_iterations=200, max_time=INF,
                       custom_limits={}, debug=False, **kwargs):
    if debug:
        import ipdb
        ipdb.set_trace()
